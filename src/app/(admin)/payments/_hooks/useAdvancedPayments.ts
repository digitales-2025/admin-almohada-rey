...
import { useGetAdvancedPaginatedPaymentsQuery } from "../_services/paymentsApi";

interface UseAdvancedPaymentsOptions {
...
export function useAdvancedPayments({
  ...
  const transformedFilters = useMemo(() => {
    ...
    Object.entries(filtersState.filters).forEach(([columnId, value]) => {
      ...
      if (backendField && value !== undefined) {
        // Para status, mantener como array de strings (no necesita conversión)
  ...
  const { data: queryData, isLoading, error, refetch } = useGetAdvancedPaginatedPaymentsQuery(queryParams);

  // Función personalizada para obtener el valor del filtro por columna
  const getFilterValueByColumnPayments = useCallback(
    (columnId: string) => {
  ...
  return {
    ...
    localSearch,
    getFilterValueByColumn: getFilterValueByColumnPayments,
  };
}
