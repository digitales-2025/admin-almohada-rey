...
import { useGetPaginatedUsersQuery } from "../_services/usersApi";

interface UseAdvancedUsersOptions {
...
export function useAdvancedUsers({
  initialPagination,
  initialFilters = {},
  initialSort = {},
  initialSearch = "",
}: UseAdvancedUsersOptions) {
    ...
    return {
      ...
      setColumnFilters: (filters: Array<{ id: string; value: any }>) => {
        if (filters.length === 0) {
          ...
          filtersActions.setFilter("rol", undefined);
          return;
        }
  ...
  const { data: queryData, isLoading, error, refetch } = useGetPaginatedUsersQuery(queryParams);

  // Función personalizada para obtener el valor del filtro por columna
  const getFilterValueByColumnUsers = useCallback(
    (columnId: string) => {
  ...
  return {
    ...
    localSearch,
    getFilterValueByColumn: getFilterValueByColumnUsers,
  };
}
