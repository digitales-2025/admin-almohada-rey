...
import { getMethodColor, getMethodIcon, getPaymentMethodLabel } from "../../../_utils/reservationPayment.utils";

interface StepConfirmationPaymentProps {
  form: UseFormReturn<CreatePaymentSchema>;
  reservation: DetailedReservation;
}

export default function StepConfirmationPayment({ form, reservation }: StepConfirmationPaymentProps) {
          ...
          {form.getValues("extraServices").length > 0 && (
            <div>
              <div className="flex items-center mb-3">
                ...
                <h4 className="font-medium">Servicios Extra</h4>
              </div>
                  ...
                  <div key={service.id} className="flex justify-between items-center ">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {service.quantity} x S/. {service.unitPrice.toFixed(2)}
                      </div>
                    </div>
                    <div className="font-medium text-sm">S/. {service.subtotal.toFixed(2)}</div>
                  </div>
                ))}
                <Separator />
                <div className="flex justify-between">
                  <span className="text-sm font-medium">Subtotal del Servicio:</span>
